        ...
        <div className="w-full max-w-xl space-y-2 sm:space-y-3">
            ...
            <form onSubmit={onSubmit} className="bg-slate-800 p-3 sm:p-5 rounded-xl border border-slate-700 space-y-3 sm:space-y-4">
                ...
                <div className="pt-2 sm:pt-3 border-t border-slate-700">
                    <h3 className="text-sm font-bold text-slate-300 mb-2 uppercase">Categoría RFEBM</h3>
                    <select
                        value={form.category || 'ADULT'}
                        onChange={(e) => {
                            const val = e.target.value as 'ADULT' | 'JUVENILE' | 'YOUTH';
                            const preset = CATEGORY_PRESETS[val];
                            setForm(f => ({
                                ...f,
                                category: val,
                                regularPeriods: preset.regularPeriods,
                                regularDuration: preset.regularDuration,
                                otDuration: preset.otDuration,
                            }));
                        }}
                        className="w-full bg-slate-900 border border-slate-700 rounded-lg p-2.5 sm:p-3 text-base sm:text-lg text-white focus:border-handball-blue outline-none mb-3"
                    >
                        <option value="ADULT">Adulto (≥16 años) — 2×30 min</option>
                        <option value="JUVENILE">Juvenil (12-16 años) — 2×25 min</option>
                        <option value="YOUTH">Infantil (8-12 años) — 2×20 min</option>
                    </select>
                </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:gap-3">
                        ...
                        <div>
                            ...
                            <select name="direction" value={form.direction} onChange={(e) => setForm(f => ({ ...f, direction: e.target.value as 'UP' | 'DOWN' }))} className="w-full bg-slate-900 border border-slate-700 rounded-lg p-2.5 sm:p-3 text-base sm:text-lg text-white focus:border-handball-blue outline-none">
                                <option value="UP">Ascendente (0 → 30)</option>
                                <option value="DOWN">Descendente (30 → 0)</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div className="pt-2 sm:pt-3 border-t border-slate-700">
                    <h3 className="text-sm font-bold text-slate-300 mb-2 uppercase">Opciones avanzadas</h3>
                    <div className="grid grid-cols-2 gap-2 sm:gap-3">
                        <label className="flex items-center gap-2 text-xs text-slate-300 cursor-pointer">
                            <input
                                type="checkbox"
                                checked={form.ot2Enabled || false}
                                onChange={(e) => setForm(f => ({ ...f, ot2Enabled: e.target.checked }))}
                                className="w-4 h-4 rounded border-slate-600"
                            />
                            Permitir 2ª prórroga
                        </label>
                        <label className="flex items-center gap-2 text-xs text-slate-300 cursor-pointer">
                            <input
                                type="checkbox"
                                checked={form.penaltyShootoutEnabled || false}
                                onChange={(e) => setForm(f => ({ ...f, penaltyShootoutEnabled: e.target.checked }))}
                                className="w-4 h-4 rounded border-slate-600"
                            />
                            Desempate por 7m
                        </label>
